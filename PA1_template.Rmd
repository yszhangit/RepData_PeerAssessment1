...
daily_total_mean=mean(daily_total$total)
daily_total_median=median(daily_total$total)
```
...
daily_total_1 <- dat1 %>% 
  group_by(date) %>% 
  summarize(total=sum(steps))
daily_total_mean_1=mean(daily_total_1$total)
daily_total_median_1=median(daily_total_1$total)
```
```{r hist1, fig.height=4}
hist((daily_total_1$total), breaks=50, main="histogram of daily total with simulated data",xlab="steps")
```  

3. Mean value of total steps each day is `r daily_total_mean_1`, median is `r daily_total_median_1`
